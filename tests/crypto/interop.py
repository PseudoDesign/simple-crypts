...
def invoke(executable, operation, secret, peer, nonce, data, valid=True):
    result = subprocess.run(
        [executable, operation, secret, peer, nonce, data],
        text=True,
        capture_output=True,
        timeout=20,
    )
...
def main():
    ...
    print(
        "PASS RFC 8032 signature vector; independent Ed25519/X25519 conversion, signatures and derived-key boxes"
    )
    ...
    for sender, receiver in (("alice", "bob"), ("bob", "alice")):
        ...
        for length in (0, 1, 15, 16, 31, 32, 64, 131, 255, 434):
            ...
            receiver_secret, sender_public = (
                vector[receiver + "_secret"],
                vector[sender + "_public"],
            )
