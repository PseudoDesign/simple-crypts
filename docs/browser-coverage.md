...
| Message lifecycle | New messages appear only in the generating endpoint; delivery or rejection moves them to the log; dropped frames enter history without a receive call; saved attempts remain replayable |
...
| Navigation and resets | Hide/reopen guidance, chapter switching with a pending frame, clean session reset, reload, retry after rejection, completed enrollment retry, credits restart, Fleet banner link from both chapters, no Sandbox navigation |
