...
with tempfile.TemporaryDirectory() as temporary:
    subprocess.run([sys.executable, 'third_party/nanopb/generator/nanopb_generator.py', '-I', 'schema', '-D', temporary, 'schema/sc.proto'], env=env, check=True)
    for name in ('sc.pb.c', 'sc.pb.h'):
        assert (Path(temporary) / name).read_bytes() == (root / 'schema' / name).read_bytes(), 'Stale generated schema: ' + name
