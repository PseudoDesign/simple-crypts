...
static sc_status seal(void *u, sc_key_handle h, const uint8_t peer[32], const uint8_t nonce[24],
                      const uint8_t *p, size_t n, uint8_t *out, size_t cap) {
    (void)h;
    (void)peer;
    (void)nonce;
    if (((memory_store *)u)->fail_seal) {
...
static sc_status open_box(void *u, sc_key_handle h, const uint8_t peer[32], const uint8_t nonce[24],
                          const uint8_t *p, size_t n, uint8_t *out, size_t cap) {
    size_t i;
    (void)h;
    (void)peer;
    (void)nonce;
    if (((memory_store *)u)->fail_open) {
        return SC_ERR_CRYPTO;
    }
    if (n < 16 || cap < n - 16) {
        return SC_ERR_BOUNDS;
...
static sc_status load(void *u, uint8_t *out, size_t cap, size_t *n, uint64_t *g) {
    ...
    memcpy(out, m->record, m->record_len);
    *n = m->record_len;
    *g = m->generation;
    return SC_OK;
}
...
static sc_status reserve(void *u, uint32_t domain, uint64_t count, uint64_t *first) {
    ...
    if (m->counters[domain] > UINT64_MAX - count) {
        return SC_ERR_EXHAUSTED;
    }
    *first = m->counters[domain];
    m->counters[domain] += count;
    return SC_OK;
}
static sc_provider provider(memory_store *m) {
    sc_provider p = {m,    public_key, seal,    open_box, random_bytes, secret,
                     load, commit,     reserve, NULL,     NULL};
    return p;
}
static void start(sc_context *ctx, memory_store *m, sc_role role, const uint8_t peer[32]) {
    ...
    if (peer) {
        memcpy(cfg.peer_public_key, peer, 32);
    }
...
static void pair(sc_context *d, memory_store *dm, sc_context *s, memory_store *sm) {
    ...
    memset(dm->key, 0x22, 32);
    memset(sm->key, 0x11, 32);
    memset(dm->token, 0x73, 32);
    memcpy(sm->token, dm->token, 32);
    start(d, dm, SC_DEVICE, sm->key);
    start(s, sm, SC_SERVER, NULL);
}
static size_t outbound(sc_context *ctx, uint8_t out[SC_MAX_FRAME]) {
    size_t n = 0;
    OK(sc_outbound(ctx, SC_MAX_FRAME, out, SC_MAX_FRAME, &n));
    CHECK(n > 78 && n <= SC_MAX_FRAME);
    return n;
}
static simplecrypts_Packet decode_packet(const uint8_t *frame, size_t n) {
    ...
    CHECK(pb_decode(&input, simplecrypts_Packet_fields, &p));
    return p;
}
static size_t rewrite_packet(uint8_t frame[SC_MAX_FRAME], const simplecrypts_Packet *p) {
    ...
    CHECK(pb_encode(&output, simplecrypts_Packet_fields, p));
    return 78 + output.bytes_written;
}
#include "schema/test_resources.h"
static void transfer(sc_context *from, sc_context *to) {
    uint8_t f[512];
    size_t n = outbound(from, f);
    OK(sc_receive(to, f, n));
}
static void enroll(sc_context *d, sc_context *s) {
    transfer(d, s);
    transfer(s, d);
    CHECK(d->state.registered && s->state.registered);
}
static void credits(void) {
    ...
    pair(&d, &dm, &s, &sm);
    CHECK(sc_consume_credits(&d, 1) == SC_ERR_ENROLLMENT);
    enroll(&d, &s);
    OK(sc_set_credits_issued(&s, 100));
    on = outbound(&s, old);
    OK(sc_receive(&d, old, on));
    OK(sc_consume_credits(&d, 25));
    n = outbound(&d, f);
    OK(sc_receive(&s, f, n));
    ...
    n = outbound(&d, f);
    start(&d, &dm, SC_DEVICE, sm.key);
    CHECK(d.state.data.groups[0].values[1].u64 == 25);
    transfer(&d, &s);
    CHECK(s.state.data.groups[0].values[1].u64 == 25);
    transfer(&s, &d);
    CHECK(sc_set_credits_issued(&s, 99) == SC_ERR_CONFLICT);
    CHECK(sc_set_credits_issued(&d, 200) == SC_ERR_ROLE);
    OK(sc_request_credit_status(&s));
    on = outbound(&s, old);
    OK(sc_request_credit_status(&s));
    transfer(&s, &d);
    ...
    transfer(&d, &s);
    transfer(&s, &d);
    OK(sc_consume_credits(&d, UINT64_MAX - 25));
    ...
    OK(sc_data_inspect(&d, 1, &g));
    CHECK(g.values[1].u64 == UINT64_MAX);
    puts("PASS credits: ownership, frozen responses, receipts, restart, bounds, failure atomicity, "
         "exact uint64, schema/version rejection");
}
static void generic(void) {
    ...
    dc = d.config;
    sc = s.config;
    dc.data_schema = &sc_example_schema;
    sc.data_schema = &sc_example_schema;
    dm.generation = 0;
    sm.generation = 0;
    dp = provider(&dm);
    sp = provider(&sm);
    ...
    memset(u, 0, sizeof u);
    u[0].field_id = 2;
    u[0].value.i64 = INT64_MIN;
    u[1].field_id = 3;
    u[1].value.boolean = 1;
    u[2].field_id = 4;
    u[2].value.length = 5;
    memcpy(u[2].value.bytes, "hello", 5);
    OK(sc_data_update_group(&s, 7, u, 3));
    transfer(&s, &d);
    CHECK(d.state.data.groups[0].values[1].i64 == INT64_MIN);
    CHECK(d.state.data.groups[0].values[2].boolean == 1);
    transfer(&d, &s);
    transfer(&s, &d);
    u[0].field_id = 5;
    u[0].value.length = 3;
    memcpy(u[0].value.bytes, "\0\xffx", 3);
    OK(sc_data_update_group(&d, 7, u, 1));
    CHECK(!d.state.pending);
    OK(sc_data_request(&s, 7));
    transfer(&s, &d);
    transfer(&d, &s);
    ...
    u[0].field_id = 1;
    CHECK(sc_data_update_group(&s, 7, u, 1) == SC_ERR_ROLE);
    OK(sc_init(&d, &dc, &dp));
    CHECK(d.state.data.groups[0].values[4].length == 3);
    OK(sc_data_request(&s, 7));
    OK(sc_data_request(&s, 8));
    {
        uint8_t a[512], b[512];
        size_t an = outbound(&s, a), bn = outbound(&s, b);
        simplecrypts_Packet ap = decode_packet(a, an), bp = decode_packet(b, bn);
        CHECK(ap.group_id != bp.group_id);
        OK(sc_receive(&d, a, an));
        OK(sc_receive(&d, b, bn));
        transfer(&d, &s);
        transfer(&d, &s);
        transfer(&s, &d);
        transfer(&s, &d);
    ...
    dc.data_schema = NULL;
    CHECK(sc_init(&d, &dc, &dp) == SC_ERR_STORAGE);
    puts("PASS generic resources: alternate schema, int64, Boolean, UTF-8, bytes, atomic "
         "validation, durable schema binding");
}
static void exhaustion(void) {
    ...
    dm.fail_reserve = 1;
    n = 0;
    CHECK(sc_outbound(&d, 512, f, 512, &n) == SC_ERR_STORAGE && n == 0);
    dm.fail_reserve = 0;
    transfer(&d, &s);
    transfer(&s, &d);
