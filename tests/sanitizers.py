...
def main() -> None:
    ...
    flags = [
        ...
        "-fno-sanitize-recover=all", "-fno-pie", "-DSC_ENABLE_TESTING",
        "-I" + str(root), "-I" + str(root / "third_party/nanopb"),
        "-I" + str(root / "third_party/libsodium/src/libsodium/include"),
    ]
    core_sources = [
        "core/sc.c", "schema/sc.pb.c", "third_party/nanopb/pb_common.c",
        "third_party/nanopb/pb_encode.c", "third_party/nanopb/pb_decode.c",
    ]
    adapter_sources = [
        "providers/sodium/sc_sodium.c", "providers/host/sc_host.c",
        "adapters/c/main.c",
    ]
    with tempfile.TemporaryDirectory(prefix="sc-sanitizers-", dir=env.get("TEST_TMPDIR")) as directory:
        output = Path(directory)

        def compile_source(source: str, extra: list[str] | None = None) -> Path:
            obj = output / (source.replace("/", "_") + ".o")
            run([compiler, *flags, *(extra or []), "-c", str(root / source), "-o", str(obj)], env=env)
            return obj

        def link(name: str, objects: list[Path], libraries: list[Path] | None = None) -> Path:
            binary = output / name
            run([compiler, "-no-pie", "-fsanitize=address,undefined",
                 "-fno-sanitize-recover=all", "-o", str(binary),
                 *map(str, objects), *map(str, libraries or [])], env=env)
            return binary

        common = [compile_source(source) for source in core_sources]
        core_test = link("core-test", common + [compile_source("tests/core_test.c")])
        core_fuzz = link("core-fuzz", common + [compile_source("tests/core_fuzz.c", ["-DSC_FUZZ_STANDALONE"])])
        adapter = link("adapter", common + [compile_source(source) for source in adapter_sources], [archive])
        ...
        run(["/usr/bin/python3", str(root / "tests/conformance.py"),
             "--device", str(adapter), "--server", str(adapter)], env=env)
