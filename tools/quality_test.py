...
class QualityTest(unittest.TestCase):
    def test_bad_format_fails(self):
        result = subprocess.run([tool('clang-format-18', '18.1.3'), '--dry-run', '--Werror',
    ...
    def test_analyzer_finding_fails(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run([tool('clang-tidy-18', '18.1.3'),
                                     '--config-file=.clang-tidy', str(source), '--', '-std=c99'],
                                    text=True, capture_output=True)
            self.assertNotEqual(result.returncode, 0, result.stdout + result.stderr)
    ...
    def documentation_fixture(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            for name in ('core/sc.h', 'core/data.h', 'providers/host/sc_host.h',
                         'providers/sodium/sc_sodium.h', 'docs/Doxyfile', 'docs/api.dox',
                         'docs/api.css', 'docs/api_example.c'):
    ...
    def docs(self, root, output):
        return subprocess.run(['/usr/bin/python3', str(Path('docs/build_api.py').resolve()),
                               '--output', str(root / output)], cwd=root,
                              text=True, capture_output=True)
    ...
    def test_documentation_failures(self):
        ...
        for declaration in declarations:
            with self.subTest(declaration=declaration), self.documentation_fixture() as root:
                ...
                self.assertNotEqual(result.returncode, 0, result.stdout + result.stderr)
                self.assertTrue('not documented' in result.stderr or
                                'Incomplete parameter' in result.stderr or
                                'unable to resolve' in result.stderr, result.stderr)
    ...
    def test_reproducible_documentation(self):
        ...
        for _ in range(2):
            with self.documentation_fixture() as root:
                ...
                inventories.append({str(p.relative_to(root / 'output')):
