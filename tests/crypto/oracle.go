...
func decode(s string, length int) []byte {
	...
	if err != nil || (length >= 0 && len(b) != length) {
...
func main() {
	if len(os.Args) != 6 {
		panic("seal|open secret peer nonce data (hex)")
	}
	...
	case "edkeys":
		...
		h[31] &= 127
		h[31] |= 64
		pub, err := curve25519.X25519(h[:32], curve25519.Basepoint)
		if err != nil {
			panic(err)
		}
		result = append(result, ed[32:]...)
		result = append(result, pub...)
		result = append(result, h[:32]...)
	case "sign":
		result = ed25519.Sign(ed25519.NewKeyFromSeed(secret[:]), data)
	case "verify":
		if len(data) < 64 || !ed25519.Verify(peer[:], data[64:], data[:64]) {
			os.Exit(2)
		}
	case "seal":
		result = box.Seal(nil, data, &nonce, &peer, &secret)
	case "open":
		var ok bool
