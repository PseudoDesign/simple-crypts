...
def read_results(path):
    ...
    if finished is None or not configured or configured != set(summaries):
        raise ValueError(
            "Incomplete BEP: every configured test must have a summary and build finish"
        )
    if not finished.get("overallSuccess"):
        raise ValueError(
            "Bazel did not finish successfully; do not replace the last verified snapshot"
        )
...
def build_report(
    bep, logs, resource_json, resource_markdown, template, output, repository, source_commit
):
    ...
    report = {
        ...
        "matrix": matrix,
        "resources": {
            "flash_bytes": resources["flash_sections_bytes"],
            "static_ram_bytes": resources["static_ram_bytes"],
            "context_bytes": context_bytes,
            "compiler": resources["compiler"],
            "report_url": "resources/cortex-m4.md",
        },
        "limitations": [
...
def verify_site(directory):
    ...
    if {(row["device"], row["server"]) for row in report["matrix"]} != expected or len(
        report["matrix"]
    ) != 16:
    ...
    if summary["targets"] != len(targets) or summary["passed"] != sum(
        t["status"] == "PASSED" for t in targets
    ):
    ...
    for row in report["matrix"]:
        ...
        if (
            ...
            or not target
            or row["status"] != target["status"]
            or row["scenario_runs"] != len(target["scenarios"])
        ):
    ...
    print(
        f"Verified report: {summary['passed']}/{summary['targets']} targets; {len(report['matrix'])} pairings"
    )
...
def main():
    ...
    parser.add_argument(
        "--resource-markdown", default="bazel-bin/platforms/cortex_m4/resource_report.md"
    )
