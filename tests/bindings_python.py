    ...
    with tempfile.TemporaryDirectory() as directory:
        with Endpoint("server", Path(directory) / "s", "python-test", bytes(32)) as server:
            ...
            with device:
                ...
                server.set_credits_issued(2**64 - 1)
            ...
            device.close()
            fails(lambda: device.consume_credits(1))
            fails(device.inspect)
        old = Path(directory) / "old"
        old.mkdir(mode=0o700)
        (old / "state.bin").write_bytes(b"SCSTORE2" + bytes(128))
        (old / "state.bin").chmod(0o600)
