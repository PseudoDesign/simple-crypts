...
def _includes(ctx):
    return depset(["."] + ctx.attr.includes, transitive = [d[CLibrary].includes for d in ctx.attr.deps])

def _compile(ctx, output, mode):
    archives = depset(transitive = [d[CLibrary].archives for d in ctx.attr.deps])
...
def conformance_matrix():
    for device in ["c", "python", "rust", "go"]:
        for server in ["c", "python", "rust", "go"]:
            ...
            py_test(name = "conformance_%s_%s" % (device, server), main = "conformance.py", data = ["coordinator.py"] + adapters, script_args = ["--device", "adapters/%s/adapter" % device, "--server", "adapters/%s/adapter" % server])
