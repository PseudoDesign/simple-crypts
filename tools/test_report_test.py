...
class ReportTest(unittest.TestCase):
    def test_complete_bep_required(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            configured = {"id": {"targetConfigured": {"label": "//tests:example"}},
                          "configured": {"testSize": "SMALL"}}
            summary = {"id": {"testSummary": {"label": "//tests:example"}},
            ...
            self.assertEqual(test_report.read_results(path)[0]["//tests:example"]["overallStatus"], "PASSED")
    ...
    def test_safe_embedding_and_paths(self):
        ...
        html = test_report.render('<script id="report-data" type="application/json">__REPORT_JSON__</script>', report)
        self.assertEqual(html.count("</script>"), 1)
        self.assertIn("\\u003c", html)
        with self.assertRaises(ValueError):
            test_report.target_path("//tests/../../etc:passwd")
        self.assertEqual(str(test_report.target_path("//tests/crypto:interop")), "tests/crypto/interop")
        self.assertEqual(test_report.SCENARIO.findall("PASS exact_uint64 seed=73821\n"), [("exact_uint64", "73821")])

    def test_evidence_manifest_and_counts(self):
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            logs = root / "logs"
            labels = [f"//tests:conformance_{d}_{s}" for d in test_report.LANGUAGES for s in test_report.LANGUAGES]
            labels += ["//tests/crypto:interop", "//tests:sanitizers", "//tests:core_fuzz_smoke"]
            events = []
            for label in labels:
                events += [{"id": {"targetConfigured": {"label": label}}, "configured": {"testSize": "SMALL"}},
                           {"id": {"testSummary": {"label": label}}, "testSummary": {"overallStatus": "PASSED", "totalRunDurationMillis": "100"}}]
            ...
            (root / "resource.json").write_text(json.dumps({"flash_sections_bytes": 99, "static_ram_bytes": 55,
                "compiler": "fixture", "data_symbols": [{"symbol": "endpoint", "bytes": 22}]}))
            (root / "resource.md").write_text("Fixture measurement")
            (root / "template.html").write_text('<script id="report-data" type="application/json">__REPORT_JSON__</script>')
            site = root / "site"
            report = test_report.build_report(root / "bep.json", logs, root / "resource.json", root / "resource.md",
                                              root / "template.html", site, "owner/repo", "a" * 40)
