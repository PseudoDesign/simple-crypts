...
static sc_status f_public(void *u, sc_key_handle key, uint8_t out[32]) {
    (void)key;
    memcpy(out, ((fuzz_store *)u)->key, 32);
    return SC_OK;
}
static sc_status f_seal(void *u, sc_key_handle key, const uint8_t peer[32], const uint8_t nonce[24],
                        const uint8_t *plain, size_t n, uint8_t *out, size_t cap) {
    (void)u;
    (void)key;
    (void)peer;
    (void)nonce;
...
static sc_status f_open(void *u, sc_key_handle key, const uint8_t peer[32], const uint8_t nonce[24],
                        const uint8_t *cipher, size_t n, uint8_t *out, size_t cap) {
    size_t i;
    (void)u;
    (void)key;
    (void)peer;
    ...
    memcpy(out, cipher + 16, n - 16);
    return SC_OK;
}
static sc_status f_secret(void *u, uint8_t out[32]) {
    memcpy(out, ((fuzz_store *)u)->token, 32);
    return SC_OK;
}
...
static sc_status f_commit(void *u, uint64_t expected, const uint8_t *record, size_t n) {
    ...
    if (expected != s->generation || n > sizeof s->record) {
        return SC_ERR_STORAGE;
    }
    memcpy(s->record, record, n);
    s->length = n;
    ++s->generation;
    return SC_OK;
}
static sc_status f_reserve(void *u, uint32_t domain, uint64_t count, uint64_t *first) {
    fuzz_store *s = u;
    if (domain > 2 || domain == 0 || s->counters[domain] > UINT64_MAX - count) {
        return SC_ERR_EXHAUSTED;
    }
    *first = s->counters[domain];
    s->counters[domain] += count;
    return SC_OK;
}
static void f_init(sc_context *ctx, fuzz_store *store, sc_role role, const uint8_t *peer) {
    sc_config config;
    sc_provider provider = {store,  f_public, f_seal,    f_open, NULL, f_secret,
                            f_load, f_commit, f_reserve, NULL,   NULL};
    memset(&config, 0, sizeof config);
    config.role = role;
    memcpy(config.serial, "fuzz-device", 12);
    if (peer) {
        memcpy(config.peer_public_key, peer, 32);
    }
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    ++data;
    --size;
    if (size > SC_MAX_FRAME) {
        return 0;
    }
    memset(&ds, 0, sizeof ds);
    memset(&ss, 0, sizeof ss);
    memset(ds.key, 0x22, 32);
    memset(ss.key, 0x11, 32);
    memset(ds.token, 0x73, 32);
    memset(ss.token, 0x73, 32);
    ...
    if (!(mode & 16u)) {
        require(sc_receive(&server, frame, n) == SC_OK);
    }
    if (mode & 2u) {
        if (!server.state.registered) {
            require(sc_receive(&server, frame, n) == SC_OK);
        }
    ...
    if (mode & 1u) {
        ...
        if (size > sizeof frame - 78u) {
            return 0;
        }
        memcpy(frame + 78, data, size);
        n = 78u + size;
    }
    if (mode & 4u) {
        ds.fail_commit = 1;
...
int main(void) {
    ...
    for (i = 0; i < 5000; ++i) {
        ...
        length = seed % sizeof data;
        for (j = 0; j < length; ++j) {
            seed = seed * 1664525u + 1013904223u;
            data[j] = (uint8_t)(seed >> 24);
        }
