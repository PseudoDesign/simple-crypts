...
def sources():
    return sorted(p for root in ROOTS for p in Path(root).iterdir()
                  if p.suffix in ('.c', '.h', '.inc'))


def tool(name, version):
    executable = str(Path(os.environ['SC_QUALITY_BIN']) / name) if os.environ.get('SC_QUALITY_BIN') else name
...
def format_check(write=False):
    ...
    subprocess.run([formatter, *(['-i'] if write else ['--dry-run', '--Werror']),
...
def lint(database):
    ...
    for record in json.loads(Path(database).read_text()):
        ...
        command = [tidy, '--config-file=.clang-tidy', record['file'], '--',
                   *C_FLAGS, *record['copts'], *['-I' + p for p in record['includes']]]
