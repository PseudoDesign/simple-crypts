...
C_FLAGS = ["-std=c99", "-O2", "-g", "-fPIC", "-Wall", "-Wextra", "-D_POSIX_C_SOURCE=200809L"]
    ...
    with tempfile.TemporaryDirectory(dir=output.parent, prefix="cc-") as tmp:
        ...
        for index, source in enumerate(config["srcs"]):
            ...
            run(
                [
                    ...
                    *C_FLAGS,
                    *config["copts"],
                    *["-I" + inc for inc in config["includes"]],
                    "-c",
                    source,
                    "-o",
                    obj,
        ...
        if config["mode"] == "archive":
            ...
            run(
                [
                    "/usr/bin/cc",
                    *(["-shared"] if config["mode"] == "shared" else []),
                    "-o",
                    str(output),
                    *objects,
                    "-Wl,--start-group",
                    *(["-Wl,--whole-archive"] if config["mode"] == "shared" else []),
                    *config["libraries"],
                    *(["-Wl,--no-whole-archive"] if config["mode"] == "shared" else []),
                    "-Wl,--end-group",
                    *config["linkopts"],
                ]
            )
...
def sodium(config):
    ...
    with tempfile.TemporaryDirectory(dir=output.parent, prefix="sodium-") as tmp:
        ...
        Path(config["stack"]).write_text(
            "".join(p.read_text() for p in sorted(source.rglob("*.su")))
        )
    ...
    with tempfile.TemporaryDirectory(dir=output.parent, prefix="sdk-") as tmp:
        ...
        if config["kind"] == "go":
            ...
            if config.get("test_build"):
                ...
                run(
                    [str(sdk / "bin/cargo"), "build", *common, "--bin", config["binary"]],
                    cwd=root,
                    env=env,
                )
    ...
    with tempfile.TemporaryDirectory(dir=outputs["probe.elf"].parent, prefix="cortex-") as tmp:
        ...
        run(
            [
                ...
                "--specs=nosys.specs",
                "-Wl,--gc-sections",
                "-Wl,-Map=" + str(outputs["probe.map"]),
                "-T",
                config["linker"],
                "-o",
                str(outputs["probe.elf"]),
                *objects,
                config["archive"],
                "-lc",
                "-lgcc",
        ...
        symbols = subprocess.check_output(
            ["/usr/bin/arm-none-eabi-nm", "-S", str(outputs["probe.elf"])], text=True
        )
        required = [
            "crypto_box_easy",
        ...
        size_text = subprocess.check_output(
            ["/usr/bin/arm-none-eabi-size", str(outputs["probe.elf"])], text=True
        )
        outputs["size.txt"].write_text(size_text)
        sizes = [int(value) for value in size_text.splitlines()[1].split()[:3]]
        stack_text = Path(config["sodium_stack"]).read_text() + "".join(
            path.read_text() for path in sorted(temporary.glob("*.su"))
        )
        ...
        for line in stack_text.splitlines():
            ...
            if len(fields) == 3:
                ...
                if function in linked:
                    stack_rows.append(
                        {"function": function, "bytes": int(fields[1]), "classification": fields[2]}
                    )
        ...
        compiler_version = subprocess.check_output([compiler, "--version"], text=True).splitlines()[
            0
        ]
        result = {
            "profile": "Cortex-M4 / Thumb / -Os / no assembly / link-only",
            "compiler": compiler_version,
            "text_bytes": sizes[0],
            "data_bytes": sizes[1],
            "bss_bytes": sizes[2],
            "flash_sections_bytes": sizes[0] + sizes[1],
            "static_ram_bytes": sizes[1] + sizes[2],
            "peak_stack_bytes": None,
            "required_symbols_retained": required,
            "largest_linked_function_stack_frames": stack_rows[:20],
        ...
        lines = [
            ...
            "| Measurement | Bytes |",
            "|---|---:|",
            "| Linked text / rodata | %d |" % sizes[0],
            "| Initialized data | %d |" % sizes[1],
            "| Zero-initialized data | %d |" % sizes[2],
            "| Text + data (flash sections) | %d |" % (sizes[0] + sizes[1]),
        ...
        lines.extend(
            "| `%s` | %d | %s |" % (row["function"], row["bytes"], row["classification"])
            for row in stack_rows[:20]
        )
