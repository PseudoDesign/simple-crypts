...
sc_status sc_sodium_seal(void *user, sc_key_handle handle, const uint8_t peer[32],
    ...
    const uint8_t *pk, *sk;
    sc_status status;
    if (!peer || !nonce || !plain || !cipher) {
        return SC_ERR_ARGUMENT;
    }
    if (length > SIZE_MAX - 16 || capacity < length + 16) {
    ...
    return crypto_box_easy(cipher, plain, (unsigned long long)length, nonce, peer, sk)
               ? SC_ERR_CRYPTO
               : SC_OK;
}
sc_status sc_sodium_open(void *user, sc_key_handle handle, const uint8_t peer[32],
    ...
    const uint8_t *pk, *sk;
    sc_status status;
    if (!peer || !nonce || !cipher || !plain) {
        return SC_ERR_ARGUMENT;
    }
    if (length < 16 || capacity < length - 16) {
        return SC_ERR_BOUNDS;
    }
    status = lookup(user, handle, &pk, &sk);
    if (status != SC_OK) {
    ...
    return crypto_box_open_easy(plain, cipher, (unsigned long long)length, nonce, peer, sk)
               ? SC_ERR_AUTH
               : SC_OK;
}
...
sc_status sc_sodium_ed_seal(void *user, sc_key_handle handle, const uint8_t peer[32],
                            ...
                            uint8_t *cipher, size_t capacity) {
    const uint8_t *pk, *sk;
    uint8_t xsk[32], xpeer[32];
    sc_status status;
    if (!peer || !nonce || !plain || !cipher || length > SIZE_MAX - 16 || capacity < length + 16) {
        return SC_ERR_BOUNDS;
    }
    status = lookup(user, handle, &pk, &sk);
    if (status != SC_OK) {
    ...
    if (crypto_sign_ed25519_pk_to_curve25519(xpeer, peer) != 0) {
        return SC_ERR_AUTH;
    }
    if (crypto_sign_ed25519_sk_to_curve25519(xsk, sk) != 0) {
...
sc_status sc_sodium_ed_open(void *user, sc_key_handle handle, const uint8_t peer[32],
    ...
    uint8_t xsk[32], xpeer[32];
    sc_status status;
    if (!peer || !nonce || !cipher || !plain || length < 16 || capacity < length - 16) {
        return SC_ERR_BOUNDS;
    }
    status = lookup(user, handle, &pk, &sk);
    if (status != SC_OK) {
...
sc_status sc_sodium_sign(void *user, sc_key_handle handle, const uint8_t *message, size_t length,
    ...
    const uint8_t *pk, *sk;
    sc_status status = lookup(user, handle, &pk, &sk);
    if (status != SC_OK) {
        return status;
    }
...
sc_status sc_sodium_verify(void *user, const uint8_t key[32], const uint8_t *message, size_t length,
    ...
    (void)user;
    return crypto_sign_verify_detached(signature, message, length, key) ? SC_ERR_AUTH : SC_OK;
}
