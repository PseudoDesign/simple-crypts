...
static sc_status secret(void *u, uint8_t p[32]) {
    (void)u;
    memset(p, 3, 32);
    return SC_OK;
}
static sc_status load(void *u, uint8_t *p, size_t capacity, size_t *n, uint64_t *g) {
    ...
    if (saved_length > capacity) {
        return SC_ERR_STORAGE;
    }
    memcpy(p, saved, saved_length);
    *n = saved_length;
    *g = generation;
    return SC_OK;
}
static sc_status commit(void *u, uint64_t g, const uint8_t *p, size_t n) {
    (void)u;
    if (g != generation || n > sizeof saved) {
        return SC_ERR_STORAGE;
    }
    memcpy(saved, p, n);
    saved_length = n;
...
void resource_probe(void) {
    ...
    strcpy(config.serial, "RESOURCE-PROBE");
    config.peer_public_key[0] = 9;
    provider.user = &keys;
    provider.public_key = sc_sodium_public;
    provider.seal = sc_sodium_seal;
    provider.open = sc_sodium_open;
    provider.random = entropy;
    provider.enrollment_secret = secret;
    provider.load = load;
