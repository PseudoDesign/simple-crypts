...
def version_assets(root):
    ...
    version = hashlib.sha256("".join(digest(root / name) for name in names).encode()).hexdigest()[
        :20
    ]
    pattern = re.compile(
        r"(['\"])(\./)?("
        + "|".join(
            re.escape(n)
            for n in sorted(set(names) | set(FLEET_NAMES), key=len, reverse=True)
            if n != "index.html"
        )
        + r")\1"
    )
    for name in names:
        if name.endswith((".html", ".mjs")):
            ...
            path.write_text(
                pattern.sub(
                    lambda m: m[1] + (m[2] or "") + m[3] + "?v=" + version + m[1], path.read_text()
                )
            )
    return version
...
def verify(root, require_commit=True):
    ...
    m = json.loads((root / "demo.json").read_text())
    if require_commit and not re.fullmatch("[0-9a-f]{40}", m["source_commit"]):
        raise ValueError("Demo needs an immutable source commit")
    ...
    expected = set(FILES if version >= 3 else legacy_fleet if version == 2 else WEB_FILES) | {
        "endpoint.wasm.mjs",
        "endpoint.wasm.wasm",
    }
    if set(m["assets"]) != expected:
        raise ValueError("Unexpected or missing demo asset inventory")
    for name, sha in m["assets"].items():
        p = (root / name).resolve()
        if not p.is_relative_to(root) or digest(p) != sha:
            raise ValueError("Changed demo asset: " + name)
    if version >= 4:
        api_assets = m.get("api_assets", {})
        actual = {p.relative_to(root).as_posix() for p in (root / "api").rglob("*") if p.is_file()}
        if (
            not {"api/index.html", "api/search/search.js"} <= set(api_assets)
            or set(api_assets) != actual
        ):
            raise ValueError("Unexpected or missing API asset inventory")
        for name, sha in api_assets.items():
            path = (root / name).resolve()
            if (
                not name.startswith("api/")
                or not path.is_relative_to(root / "api")
                or digest(path) != sha
            ):
                raise ValueError("Changed API asset: " + name)
    if (root / "endpoint.wasm.wasm").read_bytes()[:4] != b"\0asm":
        raise ValueError("Missing WebAssembly module")
    if "scw_test_" in (root / "endpoint.wasm.mjs").read_text():
        raise ValueError("Test exports in production module")
    if version >= 2:
        for module in ("server", "device"):
            prefix = root / "examples/fleet_manager" / module
            if prefix.with_suffix(".wasm").read_bytes()[:4] != b"\0asm":
                raise ValueError("Missing fleet Wasm module")
            if "scw_test_" in prefix.with_suffix(".mjs").read_text():
                raise ValueError("Test exports in fleet module")
    ...
    for name, sha in report["assets"].items():
        if digest(root / name) != sha:
            raise ValueError("Legacy evidence asset differs: " + name)
...
def assemble(
    ...
    module,
    source_commit,
    fleet="bazel-bin/examples/fleet_manager/site",
    api="bazel-bin/docs/api",
):
    root = Path(output)
    module = Path(module)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        snapshot = Path(tmp) / "report"
        shutil.copytree("site/report", snapshot)
    ...
    manifest = {
        ...
        "storage": "guided demo: temporary; fleet example: browser-local IndexedDB",
        "assets": {
            name: digest(root / name)
            for name in (*FILES, "endpoint.wasm.mjs", "endpoint.wasm.wasm")
        },
    }
    manifest["api_assets"] = {
        p.relative_to(root).as_posix(): digest(p)
        for p in sorted((root / "api").rglob("*"))
        if p.is_file()
    }
...
if __name__ == "__main__":
    ...
    p.add_argument("--verify-site")
    p.add_argument("--output")
    p.add_argument("--module", default="bazel-bin/web/endpoint.wasm.mjs")
    p.add_argument("--source-commit", default="working-tree")
    p.add_argument("--fleet", default="bazel-bin/examples/fleet_manager/site")
    p.add_argument("--api", default="bazel-bin/docs/api")
    a = p.parse_args()
    if a.verify_site:
        verify(a.verify_site)
    elif a.output:
        assemble(a.output, a.module, a.source_commit, a.fleet, a.api)
    else:
        p.error("Specify --output or --verify-site")
