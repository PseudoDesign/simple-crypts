    ...
    for path in sorted((root / 'xml').glob('*.xml')):
        ...
        for member in tree.findall('.//memberdef'):
            if member.get('kind') != 'function':
                continue
            expected = {p.findtext('declname') for p in member.findall('param')
                        if p.findtext('type') != 'void'}
            documented = {p.text for p in member.findall(
                ".//parameterlist[@kind='param']//parametername")}
            if None in expected or expected - documented:
                raise ValueError(f"Incomplete parameter documentation: {member.findtext('name')}: {expected - documented}")
...
def validate_links(root):
    ...
    site_links = {(html.parent / 'index.html').resolve(),
    ...
    for page, parsed in pages.items():
        for link in parsed.links:
            ...
            if url.fragment and target in pages and unquote(url.fragment) not in pages[target].targets:
...
def build(output):
    ...
    metadata.write_text(re.sub(
        ...
        r"\g<1>api\g<2>", metadata.read_text()))
    validate_xml(output)
