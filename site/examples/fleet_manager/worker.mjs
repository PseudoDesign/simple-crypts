 ...
 */
import createServer from './server.mjs?v=f7ac1ff01689f4d7a1b1';
import createDevice from './device.mjs?v=f7ac1ff01689f4d7a1b1';
import { Endpoint, validSerial } from './endpoint.mjs?v=f7ac1ff01689f4d7a1b1';
import { openDatabase, rows, saveRow, endpointStorage, clearFleet } from './storage.mjs?v=f7ac1ff01689f4d7a1b1';
import { exchange } from './transport.mjs?v=f7ac1ff01689f4d7a1b1';
...
async function openEndpoint(row, role, fresh) {
  return Endpoint.open(
    role === 'server' ? createServer : createDevice,
    endpointStorage(db, row.serial, role, fresh),
    role,
    row.serial,
    role === 'server' ? zeroKey : row.serverKey,
    fresh,
  );
}
...
async function synchronize(entry) {
  ...
  try {
    await exchange(entry.server, entry.device, BigInt(Math.floor(Date.now() / 1000)), (text) =>
  ...
  for (const row of await rows(db)) {
    ...
    try {
      ...
      if (row.phase !== 'ready' || row.kind !== 'browser')
        throw new Error(
          'Incomplete or incompatible saved entry. No replacement identity was created.',
        );
      entry.server = await openEndpoint(row, 'server', false);
      if (entry.server.state().public_key !== row.serverKey)
        throw new Error('Saved server identity mismatch.');
      if (row.running) entry.device = await openEndpoint(row, 'device', false);
      log(
        entry,
        row.running
          ? 'Device restored. Type help for commands.'
          : 'Device stopped. Saved state retained.',
      );
    } catch (error) {
...
if (!navigator.locks)
  rejectReady(new Error('Web Locks are required. Use HTTPS or localhost in a supported browser.'));
else
  navigator.locks
    .request('simple-crypts-fleet-v1', { ifAvailable: true }, async (lock) => {
      if (!lock)
        throw new Error('This saved fleet is open in another tab. Close that tab and reload.');
      await initialize();
      resolveReady();
...
async function create(serial) {
  validSerial(serial);
  if (savedSerials.has(serial))
    throw new Error('That serial exists in saved storage. Choose a different serial.');
...
async function dispatch({ command, serial, args = {} }) {
  ...
  switch (command) {
    case 'connection': {
      ...
      log(
        entry,
        args.enabled ? 'Connection enabled.' : 'Connection disabled. Device power unchanged.',
      );
    ...
    case 'server': {
      const labels = {
        begin: 'Server authorized enrollment.',
        cancel: 'Server canceled enrollment.',
        approve: 'Server approved the device identity.',
        issue: `Server set issued total to ${args.total}.`,
    ...
    case 'stop':
      await stop(entry);
      log(entry, 'Device stopped. Saved state retained.');
      return {};
    case 'start': {
      if (!entry.device) {
    ...
    default:
      throw new Error('Unknown fleet command.');
  }
}
